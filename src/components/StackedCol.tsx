...
const drawExample = async () => {
    ...
    const CSVData = `Entity,Code,Year,Number of patents in electric vehicle machine technology,Number of patents in electric vehicle storage,Number of patents in electric vehicle management,Number of patents in electric vehicle communication technology,Number of patents in electric vehicle charging stations
    Africa,,2001,3,,3,1,2
    Africa,,2003,1,2,1,,
    ...
    const [ xValues, yValues] = turnCSVToObject(CSVData);

    const seriesNames = ["EV Charging Stations", "EV Communication Tech", "EV Management", "EV Storage", "EV Machine Tech"];
    const fillColors = [appTheme.VividPurple, appTheme.VividPink, appTheme.VividOrange, appTheme.VividSkyBlue, appTheme.VividTeal];

    const stackedColumnCollection = new StackedColumnCollection(wasmContext);

    // Create RenderableSeries for each part of the stacked column
    for (let i = 0; i < seriesNames.length; i++) {
        const rendSeries = new StackedColumnRenderableSeries(wasmContext, {
            dataSeries: new XyDataSeries(wasmContext, { xValues: xValues[i], yValues: yValues[i], dataSeriesName: seriesNames[i] }),
            fill: fillColors[i],
            strokeThickness: 0,
            opacity: 0.8,
...
export default function StackedColumnChart() {
    ...
    return (
        <section className="graph-container">
                <p>Annual patents filed for electric vehicle technologies, World</p>
